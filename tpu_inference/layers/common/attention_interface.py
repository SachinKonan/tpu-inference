...
def sharded_ragged_paged_attention(
    ...
    # The unified pool arrives as a 6D block-major array (num_blocks,
    # num_layers, page, kvheads2//pk, pk, head_dim); per-layer caches stay 5D.
    # The fold to the kernel's 5D (num_pages, ...) happens INSIDE the shard_map
    # body: per-device, merging the two major-most dims under the default
    # layout, it compiles to a bitcast and preserves the kernel's in-place
    # aliasing. Folding it in GSPMD-traced code instead lets the SPMD
    # partitioner implement the reshape with a whole-pool relayout/reshard
    # (~2x pool -> OOM), so it must stay in here.
    ...
    def _ragged_paged_attention(*args):
        ...
        if unified_6d:
            # Per-device fold (NB_local, NL, ...) -> (NB_local*NL, ...) and
            # unfold on the way out: bitcasts, aliasing preserved. Block b's
            # layer-L page sits at flat page b*num_layers + L, matching the
            # page_indices computed in attention().
            q, k, v, kv6, *rest = args
...
def attention(
    ...
    update_kv_cache: bool = True,
    use_causal_mask: bool = True,
    layer_idx: int | None = None,  # this layer's slot in the unified cache
    num_layers: int
    | None = None,  # total layers folded into the unified cache
    ...
    if layer_idx is not None:
        # kv_cache is the unified 6D block-major cache
        #   (num_blocks, num_layers, page_size, num_kv_heads_x2//packing,
        #    packing, head_dim).
        # It is passed through to sharded_ragged_paged_attention unchanged --
        # the fold to the kernel's 5D happens per-device inside its shard_map,
        # where it is a bitcast. Here we only redirect page addressing to this
        # layer: block b's layer L page lives at flat page b*num_layers + L.
